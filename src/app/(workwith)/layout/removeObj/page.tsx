...
function Page() {
  ...
  const onSubmitHandler = async () => {
    try {
      if (!files[0]) return toast.error("Please upload an image first.");
  ...
  return (
    <div className='h-full overflow-y-auto p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className="w-full max-w-2xl mx-auto min-h-26 border border-dashed bg-white dark:bg-black border-neutral-200 dark:border-neutral-800 rounded-lg my-5">
          <FileUpload onChange={handleFileUpload} />
        </div>

        <h1 className='text-md font-bold'>Describe object to remove</h1>
        <div className="grid w-full gap-3 mt-8">
          <Textarea
            placeholder="e.g. car, chair, etc."
            id="message"
            onChange={(e) => setObject(e.target.value)}
          />
        </div>
        <p className='mt-3 text-sm font-medium'>Be specific about what you want to remove</p>
        ...
        {loading ? (
          ...
          <button
            className='bg-gradient-to-r from-orange-200 to-red-400 rounded-lg p-3 flex gap-3 items-center'
